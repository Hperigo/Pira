...
fn m_setup(app : &mut app::App) -> FrameData {
    ...
    let vao = glh::Vao::new_from_attrib_indexed(&app.gl, &attribs, &indices, &shader).expect("unable to create main vao");
...
{   
    ...
    unsafe{app.gl.disable( glow::CULL_FACE ); };
    ...
    shader.set_uniform_mat4(&app.gl,  glh::StockShader::uniform_name_perspective_matrix(),
                                            &glm::ortho(
                                                0.0,
                                                1024.0, // beacuse of mac dpi we need to scale it down
                                                768.0,
                                                0.0, 
                                                -1.0,
                                                1.0));
