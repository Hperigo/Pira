    ...
    if let event::WindowEvent::KeyboardInput { input, .. } = event {
        ...
        if matches!(input.state, event::ElementState::Released){
            if _data.transforms_arena.has_parent(_data.node_b) {
                ...
                _data.transforms_arena.set_parent(_data.node_b, _data.node_a, true);
...
fn update_fn(app : &mut piralib::app::App, data : &mut FrameData, _egui : &piralib::egui::CtxRef){
    ...
    if  transforms_arena.has_parent(data.node_b) == false {   
        transforms_arena.set_position(data.node_b, glm::vec3(app.input_state.mouse_pos.0 as f32, app.input_state.mouse_pos.1 as f32, 0.0));
    }

    // transform node a ----
    transforms_arena.set_rotation(data.node_a, glm::vec3(0.0, 0.0, app.frame_number as f32 * 0.005 ));
    ...
    transforms_arena.set_rotation(data.node_c, glm::vec3(0.0, 0.0, app.frame_number as f32 * 0.005));
