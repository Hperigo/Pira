...
impl Sphere {
    ...
    pub fn normals<'a>(&'a mut self )-> &'a mut Self {
        ...
        let data = GeometryData::gen_func(self, self.data.number_of_vertices, 3, | sphere, normal_vertices , index|{
            let vertices = &sphere.data.attribs.get( &glh::StockShader::attrib_name_position().to_string() ).unwrap().data;
        ...
        self.data.attribs.insert( glh::StockShader::attrib_name_normal().to_string(),  glh::VertexAttrib::new_normal_attr_with_data(data) );
        self
     }
}
...
impl Geometry for Sphere {
    ...
    fn get_vao_and_shader(&mut self, gl : &glow::Context) -> (glh::Vao, glh::GlslProg){
        gen_vao_and_shader(gl, glow::TRIANGLES, self.data.indices.as_ref(), &mut self.data.attribs)
    }
